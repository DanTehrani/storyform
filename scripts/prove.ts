...
import { BASE_R_SECP256K1_TEMPLATE, SECP256K1_N } from "./config";
...
const { splitToRegisters, addHexPrefix } = require("./utils");
const { downloadZKey } = require("./download-zkey");
const fs = require("fs");
const crypto = require("crypto");
const { generateMembershipProof } = require("./prove-membership");
...
const prove = async () => {
  ...
  */
  ...
  const merkleProof = await generateMembershipProof(BigInt(addHexPrefix(addr)));
  ...
  const { publicSignals, proof } = await snarkJs.groth16.fullProve(
    input,
    "./proof_of_membership_js/proof_of_membership.wasm",
    "./circuits/keys/proof_of_membership_1.zkey"
  );

  const derivedRoot = BigInt(publicSignals[0]).toString(16);
  console.log({
    root: Buffer.from(merkleProof.root).toString("hex"),
    derivedRoot
  });
